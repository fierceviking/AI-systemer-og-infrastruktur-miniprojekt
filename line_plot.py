...
exc1_df = df_spark.select("pizza_size", "order_time", "quantity")
...
#exc1_df = exc1_df.withColumn('order_datetime', concat(col('order_date'), lit(' '), col('order_time')))
 
 
#exc1_df = exc1_df.withColumn('order_time', to_timestamp(col('order_time'), 'HH:mm:ss'))  # Specify the format
...
aggregated_df = exc1_df.groupby(['pizza_size', 'hour'], as_index=False)['quantity'].sum()

total_sales_by_size = exc1_df.groupby('pizza_size', as_index=False)['quantity'].sum()

print(total_sales_by_size)

# The figure size is defined here
plt.figure(figsize=(12, 6))

# The plot is created with seaborn
sns.lineplot(data=aggregated_df, x='hour', y='quantity', hue='pizza_size', marker='o', palette='tab10')

# Boiler plate figure code
plt.title('Pizza Orders per Hour Colored by Pizza Size')
plt.xlabel('Hour of Day')
plt.ylabel('Quantity of Pizzas')
plt.xticks(range(9, 25))
#plt.legend(title='Pizza Sizes', labels=["S", "M", "L", "XL", "XXL"])
plt.grid(True)
